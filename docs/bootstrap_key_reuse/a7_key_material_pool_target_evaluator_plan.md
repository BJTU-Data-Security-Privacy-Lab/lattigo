...
target-local evaluator preparation.
...
- generate owner/target `EvaluationKeys` using existing keygen paths;
...
```bash
GOCACHE=/tmp/lattigo-gocache go test ./circuits/ckks/bootstrapping -run 'TestTargetLevelMaterial|TestKeyMaterialPool|TestTargetLevelBootstrapper|TestSharedKeyPool|TestRNSPrefix|TestBootstrapKeyReuseA[0-6]' -count=1 -timeout=30m -args -bkr.result-dir=/tmp/lattigo-bkr-a7-fast
```

- [ ] **Step 2: Run example material-size check**
...
Expected: no output except explicit non-goal wording.
